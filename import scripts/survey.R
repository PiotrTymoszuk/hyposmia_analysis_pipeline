...
# Clearing the demographics data -----
  ...
  survey_data$demo_tbl <- list(raw_survey = raw_data, 
...
# Clearing the co-morbidity data -----
  ...
  survey_data$comorb_tbl <- list(raw_survey = raw_data, 
...
# Clearing the daily medication, smoking and vaccination data -----
  ...
  survey_data$medication_tbl <- list(raw_survey = raw_data, 
...
# Clearing the CoV diagnosis data ------
  ...
  survey_data$diagn_tbl <- list(raw_survey = raw_data, 
...
# Clearing the disease course data ------
  ...
  survey_data$course_tbl <- list(raw_survey = raw_data, 
...
# Clearing the symptom data -----
  ...
  survey_data$symptom_tbl <- list(raw_survey = raw_data, 
...
# Clearing the mental health data ------
  ...
  survey_data$mental_tbl <- list(raw_survey = raw_data, 
...
# Clearing the psychosocial data -------
  ...
  survey_data$psychococ_tbl <- list(raw_survey = raw_data, 
...
# Clearing the follow-up data ------
  ...
  survey_data$follow_up <- list(raw_survey = raw_data, 
...
# clearing the vaccination readiness data -----
  ...
  survey_data$vacc_readiness <- list(raw_survey = raw_data, 
...
# Creating a single table for each cohort ------
  ...
  survey_data$north <- survey_data[c('demo_tbl', 
                               'comorb_tbl', 
                               'medication_tbl', 
                               'diagn_tbl', 
                               'course_tbl', 
  ...
  survey_data$south <- survey_data[c('demo_tbl', 
                              'comorb_tbl', 
                              'medication_tbl', 
                              'diagn_tbl', 
                              'course_tbl', 
  ...
  survey_data <- survey_data[c('north', 
                         'south')] %>% 
    map2(., names(.), 
...
# Consort data and filtering by hospitalization and observation time (test to completion) -----
  ...
  consort_data$total_recruited <- survey_data %>% 
    map(nrow)
  
  ## filtering out the hospitalized patients
  
  consort_data$hospitalized <- survey_data %>% 
    map(filter, 
  ...
  consort_data$outpatients <- survey_data %>% 
    map(nrow)
  
  ## filtering out by the 90 day cutoff
  
  consort_data$below_cutoff <- survey_data %>% 
    map(filter, 
        obs_time < 90) %>% 
    map(nrow)
  ...
  consort_data$included_analysis <- survey_data %>% 
    map(nrow)
...
# additional variable stratification tasks specific for the Psych/Soc Project -----
  ...
  source('./import scripts/data_clearing_survey.R')
