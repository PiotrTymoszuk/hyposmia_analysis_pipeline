...
# acute symptom sum stratification by quartiles, merging with the main study tables ----
  ...
  model_wrangling$sympt_sum_tbls$acute <- survey_data %>% 
    map(strat_quartile, 
...
# long symptom sum stratification by median and 75th percentile -----
  ...
  model_wrangling$sympt_sum_tbls$long <- survey_data %>% 
    map(strat_quartile, 
...
# merging the weights with the study data -----
  ...
  model_wrangling$weight_tbls$north <- survey_data$north %>% 
    select(ID, 
           sex, 
           age) %>% 
  ...
  model_wrangling$weight_tbls$south <- survey_data$south %>% 
    select(ID, 
           sex, 
           age) %>% 
    mutate(age_weight_class = cut(age, 
...
# calculating the quantile stress classes -------
  ...
  model_wrangling$stress_class <- survey_data %>% 
    map(strat_quartile, 
...
# Acute and Persistent symptom classification as psychosomatic, counting, stratifying by quartiles -----
  ...
  model_wrangling$psychosom_acute_sympt_sum <- survey_data %>% 
    map(sum_variables, 
        vars_to_sum = paste(model_wrangling$psychosom_sympt, 
                            'acute', sep = '_'))
  
  model_wrangling$psychosom_long_sympt_sum <- survey_data %>% 
    map(sum_variables, 
        vars_to_sum = paste(model_wrangling$psychosom_sympt, 
                            'long', sep = '_'))
  ...
  model_wrangling$psychosom_acute_sympt_class <- survey_data %>% 
    map(strat_quartile, 
        numeric_variable = 'psychosom_acute_sympt_sum', 
        labels = c('1Q', '2Q', '3Q', '4Q'), 
        new_var_name = 'psychosom_acute_sympt_class')
  
  model_wrangling$psychosom_long_sympt_class <- list(inp_tbl = survey_data, 
...
# neurocognitive symptoms -----
  ...
  model_wrangling$neurocognitive_acute_sympt_sum <- survey_data %>% 
    map(sum_variables, 
        vars_to_sum = paste(model_wrangling$neurocognitive_sympt, 
                            'acute', sep = '_'))
  
  model_wrangling$neurocognitive_long_sympt_sum <- survey_data %>% 
    map(sum_variables, 
        vars_to_sum = paste(model_wrangling$neurocognitive_sympt, 
                            'long', sep = '_'))
