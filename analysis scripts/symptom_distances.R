...
# globals -----
  ...
  sympt_dist$covild_tbl <- mod_tbl$covild %>% 
    dlply(c('cat_WHO', 'time_numeric'), select, -time_numeric, -cat_WHO) %>% 
    map(~filter(.x, complete.cases(.x))) %>% 
...
# Matrices with the SMC distances -----
  ...
  sympt_dist$dist_hact <- sympt_dist$hact_tbl %>% 
    map(calculate_dist, 'smc')
  
  sympt_dist$dist_covild <- sympt_dist$covild_tbl %>% 
    map(calculate_dist, 'smc')
...
# Multi-dimensional scaling -------
  ...
  sympt_dist$mds_hact <- sympt_dist$hact_tbl %>% 
    map(reduce_data, 
        distance_method = 'smc',
        kdim = 2, 
        red_fun = 'mds')
  
  sympt_dist$mds_covild <- sympt_dist$covild_tbl %>% 
    map(reduce_data, 
        distance_method = 'smc', 
        kdim = 2, 
        red_fun = 'mds')
...
# Two-dimensional MDS plots, HACT ------
  ...
  sympt_dist$mds_plots_hact <- list(x = sympt_dist$mds_hact, 
  ...
  sympt_dist$mds_plots_hact <- list(x = sympt_dist$mds_plots_hact, 
    ...
    pmap(function(x, y, z) x + 
           labs(title = y, 
                subtitle = z) + 
           geom_text_repel(aes(label = translate_var(observation, 
                                                     dict = hact$dict), 
...
# Two-dimensional MDS plots, CovILD ------
  ...
  sympt_dist$mds_plots_covild <- list(x = sympt_dist$mds_covild, 
  ...
  sympt_dist$mds_plots_covild <- list(x = sympt_dist$mds_plots_covild, 
                                    y = rep(c('Acute CoV', 
                                              '2-month FUP', 
                                              '3-month FUP', 
                                              '6-month FUP', 
...
# mean 5-NN, HACT -------
  ...
  sympt_dist$mean_kNN_hact <- list(north = sympt_dist$mean_kNN_hact[1:4], 
                                   south = sympt_dist$mean_kNN_hact[5:8]) %>% 
    map(~map2_dfr(., c(3, 14, 28, 90), ~mutate(.x, time = .y))) %>% 
    map(mutate, symptom = translate_var(variable, dict = hact$dict))
...
# mean 5-NN distances, CovILD -------
  ...
  sympt_dist$mean_kNN_covild <- list(ambulatory = sympt_dist$mean_kNN_covild[1:5], 
    ...
    map(mutate, symptom = translate_var(variable, dict = covild$dict))
...
# Displaying the kNN distance in the plots -----
  ...
  sympt_dist$kNN_plots_covild <- list(data = sympt_dist$mean_kNN_covild, 
